...
use Exception;
use Chifoidriss\Notifer\Exceptions\MissingPresetNotificationException;
use Chifoidriss\Notifer\Storage\Session;

class LaravelNotifer
{
    protected Session $session;

    /**
     * Create a new notifer instance.
     *
     ...
     * @return LaravelNotifer
     * @throws Exception
     */
    public function preset(string $presetName, array $overrideValues = []): self
    {
        $presetValues = config('notifer.preset-messages.'.$presetName);
    ...
    public function flash(string $message, $type = null, $icon = null, string $model = null, string $title = null): void
    {
        ...
        $this->session->flash('notifer', $notifications);
    }
    ...
    {
        return $this->session->get('notifer.message');
    }
    ...
    {
        return $this->session->get('notifer.type');
    }
}
